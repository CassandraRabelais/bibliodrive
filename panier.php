...
<body>
    ...
    <div class="container mt-4">
        ...
        <?php else: ?>
            <div class="row">
                <?php foreach ($cartBooks as $book): ?>
                    <div class="col-md-4 mb-3">
                        <div class="card">
                            <img src="covers/<?php echo htmlspecialchars($book['photo']); ?>" class="card-img-top" alt="<?php echo htmlspecialchars($book['titre']); ?>">
                            <div class="card-body">
                                <h5 class="card-title"><?php echo htmlspecialchars($book['titre']); ?></h5>
                                <p class="card-text">Auteur: <?php echo htmlspecialchars($book['prenom'] . ' ' . $book['nom']); ?></p>
                                <form method="post" style="display: inline;">
                                    <input type="hidden" name="nolivre" value="<?php echo $book['nolivre']; ?>">
                                    <button type="submit" name="remove" class="btn btn-danger">Retirer</button>
                                </form>
                            </div>
                        </div>
                    </div>
            ...
            <?php else: ?>
                <p class="alert alert-warning">Vous devez être connecté pour emprunter.</p>
            <?php endif; ?>
        <?php endif; ?>
    </div>
